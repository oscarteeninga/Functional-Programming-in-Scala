...
object Anagrams extends AnagramsInterface {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = (w.toLowerCase.toSeq groupBy (ch => ch) map { case (ch, list) => (ch, list.length)}).toList.sorted
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    x map { case (ch, occ) =>
      val d_occ = occ - (y.toMap withDefaultValue 0)(ch)
      if (d_occ >= 0) ch -> d_occ
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def sentenceAnagram(occ: Occurrences): List[Sentence] = occ match {
      case Nil => List(List())
      case _::_ => for {
        comb <- combinations(occ)
        matches <- (dictionaryByOccurrences withDefaultValue(List()))(comb)
        acc <- sentenceAnagram(subtract(occ, comb))
      } yield matches :: acc
    }
    sentenceAnagram(sentenceOccurrences(sentence))
  }
}
